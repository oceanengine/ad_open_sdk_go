...
func main() {
	...
	var request ApiOpenApi2ToolsClueGetGetRequestExample
	...
	resp, httpRes, err := apiClient.ToolsClueGetV2Api().
		...
		AdvertiserIds(request.AdvertiserIds).StartTime(request.StartTime).EndTime(request.EndTime).Page(request.Page).PageSize(request.PageSize).
		Execute()
