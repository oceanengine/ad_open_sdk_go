...
func main() {
	...
	resp, httpRes, err := apiClient.SpiTaskGetV2Api().
		...
		AppId(request.AppId).EndDate(request.EndDate).Page(request.Page).PageSize(request.PageSize).ServiceLabel(request.ServiceLabel).StartDate(request.StartDate).Status(request.Status).SubscribeId(request.SubscribeId).
		Execute()
