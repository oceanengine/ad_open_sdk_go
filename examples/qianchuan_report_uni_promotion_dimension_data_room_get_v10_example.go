...
func main() {
	...
	resp, httpRes, err := apiClient.QianchuanReportUniPromotionDimensionDataRoomGetV10Api().
		...
		AdvertiserId(request.AdvertiserId).RoomId(request.RoomId).StartTime(request.StartTime).EndTime(request.EndTime).Dimension(request.Dimension).Metrics(request.Metrics).OrderField(request.OrderField).OrderType(request.OrderType).Page(request.Page).PageSize(request.PageSize).
		Execute()
