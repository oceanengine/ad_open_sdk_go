...
func main() {
	...
	resp, httpRes, err := apiClient.EventManagerOptimizedGoalGetV2V30Api().
		...
		AdvertiserId(request.AdvertiserId).LandingType(request.LandingType).AdType(request.AdType).AssetType(request.AssetType).AssetId(request.AssetId).PackageName(request.PackageName).AppType(request.AppType).AppPromotionType(request.AppPromotionType).MarketingGoal(request.MarketingGoal).QuickAppId(request.QuickAppId).DeliveryMode(request.DeliveryMode).MiniProgramId(request.MiniProgramId).DpaAdtype(request.DpaAdtype).MicroPromotionType(request.MicroPromotionType).MicroAppInstanceId(request.MicroAppInstanceId).
		Execute()
