...
func main() {
	...
	resp, httpRes, err := apiClient.QianchuanSuggestBidV10Api().
		...
		AdvertiserId(request.AdvertiserId).MarketingGoal(request.MarketingGoal).ExternalAction(request.ExternalAction).AwemeId(request.AwemeId).ProductId(request.ProductId).CampaignScene(request.CampaignScene).
		Execute()
