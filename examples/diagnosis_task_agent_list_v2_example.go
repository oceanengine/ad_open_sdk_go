...
func main() {
	...
	resp, httpRes, err := apiClient.DiagnosisTaskAgentListV2Api().
		...
		AgentId(request.AgentId).Results(request.Results).Status(request.Status).StartTime(request.StartTime).EndTime(request.EndTime).Page(request.Page).PageSize(request.PageSize).
		Execute()
