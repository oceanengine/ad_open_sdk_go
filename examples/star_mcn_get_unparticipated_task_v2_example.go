...
func main() {
	...
	resp, httpRes, err := apiClient.StarMcnGetUnparticipatedTaskV2Api().
		...
		StarId(request.StarId).Page(request.Page).PageSize(request.PageSize).PayType(request.PayType).MinCreateTimeStamp(request.MinCreateTimeStamp).MaxCreateTimeStamp(request.MaxCreateTimeStamp).DeveloperId(request.DeveloperId).SettlementType(request.SettlementType).
		Execute()
