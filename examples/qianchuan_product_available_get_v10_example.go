...
type ApiOpenApiV10QianchuanProductAvailableGetGetRequestExample struct {
	...
	CampaignScene QianchuanProductAvailableGetV10CampaignScene `json:"campaign_scene,omitempty"`
...
func main() {
	...
	resp, httpRes, err := apiClient.QianchuanProductAvailableGetV10Api().
		...
		AdvertiserId(request.AdvertiserId).AwemeId(request.AwemeId).Filter(request.Filter).CampaignScene(request.CampaignScene).Page(request.Page).PageSize(request.PageSize).Cursor(request.Cursor).
		Execute()
