...
// All allowed values of ToolsNoBidSuggestBidV2ExternalAction enum
var AllowedToolsNoBidSuggestBidV2ExternalActionEnumValues = []ToolsNoBidSuggestBidV2ExternalAction{
...
// NewToolsNoBidSuggestBidV2ExternalActionFromValue returns a pointer to a valid ToolsNoBidSuggestBidV2ExternalAction
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewToolsNoBidSuggestBidV2ExternalActionFromValue(v string) (*ToolsNoBidSuggestBidV2ExternalAction, error) {
	ev := ToolsNoBidSuggestBidV2ExternalAction(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for ToolsNoBidSuggestBidV2ExternalAction: valid values are %v", v, AllowedToolsNoBidSuggestBidV2ExternalActionEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v ToolsNoBidSuggestBidV2ExternalAction) IsValid() bool {
	for _, existing := range AllowedToolsNoBidSuggestBidV2ExternalActionEnumValues {
		if existing == v {
			return true
		}
	}
